...
in
    ...
    fun {ChargeAllowed WeaponList State}
        Wp
    in  
        if {List.length WeaponList} > 0 then
            Wp = {List.nth WeaponList ({OS.rand} mod {List.length WeaponList}) + 1}

            if State.Wp < Input.Wp then
                Wp
            else
                {ChargeAllowed {List.subtract WeaponList Wp} State}
            end
        else
            null
        end
    ...
    fun {MineOnRange Players Nb Mines MyID MyPos}
        fun {NearPlayer Players ID Mine MyID MyPos}
            Dist MyDist
        in
            if ID \= MyID andthen {Record.width Players} > 0  andthen ID =< Input.nbPlayer andthen {Value.hasFeature Players ID} andthen Players.ID.x \= 0  then
                ...
                if Dist =< 1 andthen MyDist > 1 then
                    ...
                    {NearPlayer Players ID+1 Mine MyID MyPos}
                end
    ...
    in
        if {List.length Mines} > 0 andthen Nb =< {List.length Mines} then
            case {NearPlayer Players 1 {List.nth Mines Nb} MyID MyPos} of true then
                {List.nth Mines Nb}
            [] false then {MineOnRange Players Nb+1 Mines MyID MyPos}
            end
        else
            null
        end
    ...
    proc{TreatStream Stream ID Position Weapons Surface Direction Mines Life Players Prey Item} % as as many parameters as you want
        ...
        in
            ...
            if Item == nil orelse (Weapons.Item mod Input.Item) == 0 then
                TmpW = {ChargeAllowed [missile mine drone sonar] Weapons}
            else
                TmpW = Item
            end
        ...
        in
            ...
            KindFire = {FireRandom [drone sonar mine ] Weapons Position.1}
        ...
        in
            ...
            ID2 = ID
            case {MineOnRange Players 1 Mines ID.id Position.1} of pt(x:X y:Y) then
                Mine = pt(x:X y:Y)
        ...
        in
            ...
            case Dist of 0 then
                HP = Life - 2
                {System.show [ID2.color a toucher ID.color avec missile Life HP]}
                if HP > 0 then
        ...
        in
            ...
            Dist = {Abs (Pos.x - Position.1.x)} + {Abs (Pos.y - Position.1.y)}
            case Dist of 0 then
                {System.show [player1 ID2.color a toucher ID.color avec mine]}
                HP = Life - 2
                if HP > 0 then
                    Message = sayDamageTaken(ID 1 HP)
                else
