...
in
    ...
    in
        case Messages of H|T then
            ...
            [] sayDeath(ID) then
                ...
                Alives = {HowMuchAlive PortsPlayers 0}
                if Alives > 1 then
                    {Send PortGUI sound(dead)}
                end
                {Remaining T PPorts}
            end
        [] nil then
    ...
    in
        if Current =< Input.nbPlayer then
            ...
            if Answer andthen {Not Input.isTurnByTurn} then
                skip
            else
